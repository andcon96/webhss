                ...
                <form class="form-horizontal" method="post" action="{{ route('usermaint.store') }}">
                    ...
                    <div class="modal-body">
                        <div class="form-group row">
                            ...
                            <div class="col-md-7">
                                <select id="domain" class="form-control role" name="domain" autofocus>
                                    <option value=""> Select Data </option>
                                    @foreach ($domain as $domains)
                                        <option value="{{ $domains->id }}">{{ $domains->domain_code }} --
                                            {{ $domains->domain_desc }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            <label for="password-confirm" class="col-md-3 col-form-label text-md-right">Confirm
                                Password</label>
                            <div class="col-md-5">
                        ...
                        <div class="form-group row">
                            <label for="backdate" class="col-md-3 col-form-label text-md-right">Can Backdate</label>
                            <div class="col-md-7">
                                <input type="checkbox" style="margin-top:12px;" id="backdate" name="backdate"
                                    value="1">
                            </div>
                        </div>
                    </div>
                ...
                <form class="form-horizontal" method="post" action="{{ route('usermaint.update', 'Edit') }}">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-7 {{ $errors->has('d_uname') ? 'has-error' : '' }}">
                                <input id="d_uname" type="text" class="form-control" name="d_uname"
                                    value="{{ old('d_uname') }}" readonly autofocus>
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="d_name" class="col-md-3 col-form-label text-md-right">Nama User</label>
                            <div class="col-md-7">
                                <input id="d_name" type="text" class="form-control" autocomplete="off"
                        ...
                        <div class="form-group row">
                            <label for="d_backdate" class="col-md-3 col-form-label text-md-right">Can Backdate</label>
                            <div class="col-md-7">
                                <input type="checkbox" style="margin-top:12px;" id="d_backdate" name="d_backdate"
                                    value="1">
                            </div>
                        </div>
                    </div>
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('usermaint.destroy', 'Delete') }}" method="post">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="container">
                            <div class="row">
                                Are you sure you want to &nbsp; <a name="temp_status" id="temp_status"></a> &nbsp; user
                                :&nbsp; <a name="temp_uname" id="temp_uname"></a> &nbsp;?
                            </div>
                        </div>
                ...
                <form class="form-horizontal" method="post" action="/adminchangepass">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-7 {{ $errors->has('d_uname') ? 'has-error' : '' }}">
                                <input id="c_uname" type="text" class="form-control" name="c_uname"
                                    value="{{ old('d_uname') }}" readonly autofocus>
                            </div>
                        </div>
                        <div class="form-group row">
                        ...
                        <div class="form-group row">
                            <label for="c_password-confirm" class="col-md-3 col-form-label text-md-right">Confirm
                                Password</label>
                            <div class="col-md-6">
                                <input id="c_password-confirm" type="password" class="form-control"
                                    name="password_confirmation" required>
                            </div>
                        </div>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.editUser', function() { // Click to only happen on announce links
            ...
            var d_backdate = $(this).data('d_backdate');
            ...
            document.getElementById("d_backdate").checked = d_backdate == 1 ? true : false;
            ...
            jQuery.ajax({
                ...
                success: function(data) {
                    console.log(data);
                    $('#t_roletype').find('option').remove().end().append(
                        '<option value="">Select Data</option>');
                    for (var i = 0; i < data.length; i++) {
                        if (role_type == data[i].role_type) {
                            $('#t_roletype').append('<option value="' + data[i].id + '" selected>' +
                                data[i].role_type + '</option>');
                        } else {
                            $('#t_roletype').append('<option value="' + data[i].id + '">' + data[i]
                                .role_type + '</option>');
                        }
                    }
                }
        ...
        $(document).ready(function() {
            $("#role,#roletype,.roletype,#domain").select2({
                width: '100%'
            });
            ...
            $("#role").change(function() {
                ...
                jQuery.ajax({
                    ...
                    success: function(data) {
                        console.log(data);
                        $('#roletype').find('option').remove().end().append(
                            '<option value="">Select Data</option>');
                        for (var i = 0; i < data.length; i++) {
                            if (data[i].role_type == 'Super_User') {
                                $('#roletype').append('<option value="' + data[i].id +
                                    '">Super User</option>');
                            } else {
                                $('#roletype').append('<option value="' + data[i].id + '">' +
                                    data[i].role_type + '</option>');
