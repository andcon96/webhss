...
class CreateInvoicePricesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('invoiceprice', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('ip_shipfrom_id')->index()->nullable();
            $table->foreign('ip_shipfrom_id')->references('id')->on('shipfrom');
            $table->unsignedBigInteger('ip_customership_id')->index();
            $table->foreign('ip_customership_id')->references('id')->on('customership');
